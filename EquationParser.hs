...
-- Initial (start of the line) = 1, ReadElement (read either a positive or negative number or symbol name) = 2, ReadCoefficient (read number) = 3, ReadSymbol (symbol name) = 4, ReadRightSide (after equal sign) = 5, ReadNegativeFreeMember (number) = 6, ReadPositiveFreeMember (number) = 7
data ParserState = Initial | ReadElement | ReadCoefficient | ReadSymbol | ReadRightSide | ReadNegativeFreeMember | ReadPositiveFreeMember
...
parseRow (c:cs) state coefficients var_names
	| state == Initial && (c == '-') = parseRow cs ReadElement ((-1) : coefficients) var_names
	| state == Initial && isNum c = parseRow cs ReadCoefficient (c_int : coefficients) var_names
	| state == Initial && isAlpha c = parseRow cs ReadSymbol (1 : coefficients) ([c] : var_names)
